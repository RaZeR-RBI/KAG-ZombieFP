	...
	if (blob.getMap().getBlobsInRadius( pos, SEEK_RANGE, @blobsInRadius ))
	{
		if (strategy == Strategy::find_teammate || strategy == Strategy::idle) 
		{
			...
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				...
				if (b !is blob && b.getTeamNum() == blob.getTeamNum() && !b.isInFlames() && !b.isInWater() && (b.hasTag("human_player") || b.hasTag("bed")))
				{								  
					// omit full beds or when bot
					const string name = b.getName();
					if (name == "dorm" && (blob.getPlayer() !is null || isRoomFullOfMigrants(b))) {
						//printf("Dorm, room full or player is null");
						continue;
					}			 
...
CBlob@ getAttacker( CBrain@ this, CBlob @blob )
{
	...
	if (map.getBlobsInRadius( pos, ENEMY_RANGE, @blobsInRadius ))
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			CBlob @b = blobsInRadius[i];
			if (b !is blob 
				&& (((b.getTeamNum() != blob.getTeamNum() && (b.hasTag("player") || b.hasTag("zombie")) && !b.hasTag("migrant") && !b.hasTag("dead")) || (b.isInFlames() || b.hasTag("animal"))) 	// runaway from enemies and from burning stuff
...
void GoToBlob( CBrain@ this, CBlob @target )
{
	...
	if (targetDistance < 100.0f && target.hasTag("human_player") && (getGameTime() % 100 == 0) && XORRandom(10)==0) // keep distance from player
	{
		...
		blob.Chat("Place me in a dorm so I can let players heal and spawn in them!");
		else if (r==2)
		blob.Chat("Trying to get to a dorm so players can heal and spawn there!");
	}
