...
void onInit(CBlob@ this)
{
	...
	this.set_string("autograb blob", "mat_rockets");
	...
	if (getNet().isServer())
	{
		CBlob@ ammo = server_CreateBlob("mat_rockets");
		if (ammo !is null)
		{
...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _unused)
{
	if (bullet !is null)
	{
		...
		Vec2f vel = Vec2f(charge / 24.0f * (this.isFacingLeft() ? -1 : 1), 0.0f).RotateBy(angle);
		bullet.setVelocity(vel);
		Vec2f offset = arm_offset;
		offset.RotateBy(angle);
		bullet.setPosition(this.getPosition() + Vec2f(4,-4) *1.1f);
